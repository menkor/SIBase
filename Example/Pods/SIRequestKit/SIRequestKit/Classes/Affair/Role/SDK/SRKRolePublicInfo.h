//
//  Generated by the swagger2objc.  DO NOT EDIT!
//  SRKRolePublicInfo.h
//  Super Id
//
//  Copyright © 2018 SiMu. All rights reserved.
//

#import "SIUserRolePublicInfoItem.h"
#import <Foundation/Foundation.h>

/**
 path       : /affair/role/public_info
 method     : GET
 summary    : 得到userId的公开角色信息
 type       : UserRolePublicInfoVO
 response   : SIUserRolePublicInfoItem
*/
@interface SRKRolePublicInfo : NSObject

/**
 paramType  : header
 key        : X-SIMU-RoleId
 type       : int64
 required   : true
 notes      : X-SIMU-RoleId
 rename     : X_SIMU_RoleId
*/
@property (nonatomic, strong, nullable) NSNumber /*int64_t*/ *X_SIMU_RoleId;

/**
 paramType  : header
 key        : X-SIMU-AffairId
 type       : int64
 required   : true
 notes      : X-SIMU-AffairId
 rename     : X_SIMU_AffairId
*/
@property (nonatomic, strong, nullable) NSNumber /*int64_t*/ *X_SIMU_AffairId;

/**
 paramType  : header
 key        : X-SIMU-UserId
 type       : int64
 required   : true
 notes      : X-SIMU-UserId
 rename     : X_SIMU_UserId
*/
@property (nonatomic, strong, nullable) NSNumber /*int64_t*/ *X_SIMU_UserId;

/**
 paramType  : query
 key        : targetUserId
 type       : int64
 required   : true
 notes      : targetUserId
*/
@property (nonatomic, strong, nullable) NSNumber /*int64_t*/ *targetUserId;

/**
 The class of response object[s]
 @return [SIUserRolePublicInfoItem class]*/
+ (Class /*SIUserRolePublicInfoItem*/ __nullable)responseClass;

- (NSDictionary *__nonnull)srk_config;

@end
