...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton setTitleColor:[SIColor colorWithHex:0x4a4a4a] forState:UIControlStateNormal];
...
- (void)textDidChangeNotify:(NSNotification *)notify {
    if ([notify.object isEqual:self.textField]) {
        if ([self.delegate respondsToSelector:@selector(searchView:searchActionKeyText:searchString:)]) {
...
- (void)setKeyText:(NSString *)keyText {
    ...
    NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:self.keyText attributes:@{NSFontAttributeName: [SIFont systemFontOfSize:12], NSForegroundColorAttributeName: [SIColor colorWithHex:0x926dea]}];
